...
<div class="col-md-6 grid-margin stretch-card">
    ...
    <div class="form-group">
        ...
        <input type="number" class="form-control" id="contact_number" name="contact_number" value="{{old('contact_number')}}" autocomplete="off" placeholder="Contact Number">
    </div>

     <div class="form-group">
        <label class="control-label">Full Name</label>
        <input type="text" class="form-control" id="full_name" name="full_name" value="{{old('full_name')}}" autocomplete="off" placeholder="Full Name">
    </div>

   <div class="form-check form-check-flat form-check-primary">
                                        <label class="form-check-label">
                                            <input type="checkbox" name="keep_identity_concealed" value="1" class="form-check-input" {{(old('keep_identity_concealed') == "1") ? 'checked': ''}}>
...
<select class="js-example-basic-single w-100" name="beat" id="beatdrop">
     <option selected="" disabled="">Select Beat</option>
        ...
        <label class="control-label">Beat</label>
          ...
          <input type="text" class="form-control" id="beat" name="beat" value="{{old('beat')}}" autocomplete="off" placeholder="Beat">
    </div>

     <div class="form-group">
        <label class="control-label">Block</label>
       
          <input type="text" class="form-control" id="block" name="block" value="{{old('block')}}" autocomplete="off" placeholder="Block">
    </div>

     <div class="form-group">
        <label class="control-label">Range</label>
       
         <input type="text" class="form-control" id="range" name="range" value="{{old('range')}}" autocomplete="off" placeholder="Range">
    </div>

      <div class="form-group">
        <label class="control-label">District</label>
        <input type="text" class="form-control" id="district" name="district" value="{{old('district')}}" autocomplete="off" placeholder="District">
    </div>

     <div class="form-group">
        <label class="control-label">Division</label>
       
        <input type="text" class="form-control" id="division" name="division" value="{{old('division')}}" autocomplete="off" placeholder="Division">
    </div>
     ...
     <div class="form-group">
        ...
        <select class="form-control" name="complaint" id="complaint">
            ...
            @if (old('complaint') == $complainttype->complaint_type)
            ...
            @endforeach
                                       ...
                                       <textarea class="form-control" name="information" id="information" rows="5">{{old('information')}}</textarea>
 ...
 <label class="control-label">Complaint Detail</label>
                                       <textarea class="form-control" name="complaintdetail" id="complaintdetail" rows="5">{{old('complaintdetail')}}</textarea>
    ...
    $("input[name$='nature_information']").click(function() {
        ...
        $(document).ready(function () {
           $('#beatdrop').change(function () {
             var id = $(this).val();
             $.ajax({
                url:'/complaintportal/ForestareasModule/getforestarea/'+id,
                type:'get',
                dataType:'json',
        ...
        $(document).ready(function () {
           $('#villagedrop').change(function () {
             var id = $(this).val();
             $.ajax({
                url:'/complaintportal/VillagesModule/getvillage/'+id,
                type:'get',
                dataType:'json',
                success:function (response) {
                    var len = 0;
